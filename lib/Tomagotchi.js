...
class Pet {
	...
	increase_loved (int) {
		const amt = int || 5
		this.loved + amt >= 100 ? this.loved = 100 : this.loved += amt
		console.log(`Increasing loved: ${this.loved}`)
		return this.loved
	}
	reduce_bored(int) {
		const amt = int || 1
	...
	reduce_loved(int) {
		const amt = int || 5
		this.loved - amt <= 0 ? this.loved = 0 : this.loved -= amt
		console.log(`Reducing loved: ${this.loved}`)
		return this.loved
	}
	...
	play (game) {
		this.reduce_bored()
		chance(0.5) ? this.increase_hunger(10) : this.increase_hunger() 
	}
